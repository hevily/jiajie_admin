          ...
          render: (h, params) => {
            if (params.row['config_var_type'] === 'bool') {
              return h('i-switch', {
                ...
                on: {
                  'on-change': value => {
                    this.$http(`config.setting-${params.row['config_key']}`, {config_value: value}).then(rs => {
                      if (rs.error === 0) {
              ...
              return h('div', [
                h('Input', {
                  ...
                  on: {
                    'on-change': value => {
                      if (value.data) {
                        params.row['config_value'] = value.data
                      }
                    }
                  }
                }),
                h('Button', {
                  ...
                  on: {
                    'click': value => {
                      if (params.row['config_value']) {
                        this.$http(`config.setting-${params.row['config_key']}`,{config_value: params.row['config_value']}).then(rs => {
                          if (rs.error === 0) {
                            this.$Notice.success({desc: '提交成功'})
                          }
                        })
